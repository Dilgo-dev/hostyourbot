...
export const getUserSubscription = async (): Promise<UserSubscription> => {
  const response = await authApi.get('/api/auth/subscription/me');
  return response.data.subscription;
};

export const getResourceLimits = async (): Promise<SubscriptionLimits> => {
  const response = await authApi.get('/api/auth/subscription/limits');
  return response.data.limits;
};

export const canCreateBot = async (): Promise<boolean> => {
  const response = await authApi.get('/api/auth/subscription/can-create-bot');
